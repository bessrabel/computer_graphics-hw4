...
class Gouraudshading{
    ...
    public:
        Gouraudshading(int Nx, int Ny, Shade shade) : Nx(Nx), Ny(Ny), shade(shade) {};

        unsigned char* getImage(){ return Image; }

        void gouraudShading(Modeling model, Camera cam, PerspectiveProjection projection, Viewport view){
            ...
            for(int i = 0; i < gNumTriangles; i++){
                ...
                // normal (per-vertex) and Shade using per-vertex normal
                vec3 e = cam.getE();
                vec3 color_a = shade.gouraudShade(e, a, model.getCenter());
                vec3 color_b = shade.gouraudShade(e, b, model.getCenter());
                vec3 color_c = shade.gouraudShade(e, c, model.getCenter());
                    ...
                    for(int x = xmin; x <= xmax; x ++){
                        if (beta > 0 && gamma > 0 && beta + gamma < 1){
                            ...
                            if(depth > depthBuffer[y * Nx + x]) { // Write color if depth test passes
                                ...
                                vec3 color = (1-(beta+gamma)) * color_a + beta * color_b + gamma * color_c; // // Interpolate color
                                for (int i = 0; i < 3; i ++){
                                    int idx = (y * Nx + x) * 3;
